            ...
            while (true) {
                try {
                    ...
                    if (obj instanceof ProductInfo) {
                        ...
                        zookeeperDistributedLock.lock(LockPrefix.PRODUCT_LOCK_PREFIX + productInfo.getId());
                        ...
                        zookeeperDistributedLock.unlock(LockPrefix.PRODUCT_LOCK_PREFIX + productInfo.getId());
                    } else if (obj instanceof ShopInfo) {
                        ShopInfo shopInfo = (ShopInfo) obj;
                        log.info("get msg from rebuildCacheQueue. {}", shopInfo);
                        zookeeperDistributedLock.lock(LockPrefix.SHOP_LOCK_PREFIX + shopInfo.getId());
                        //  成功获取到锁后，先从redis获取数据，比较update_time,再判断是否更新redis数据
                        ...
                        zookeeperDistributedLock.unlock(LockPrefix.SHOP_LOCK_PREFIX + shopInfo.getId());
                    } else {
